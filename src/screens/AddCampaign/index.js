...
const AddCampaign = ({ navigation }) => {
  ...
  const btnSubmittedStatus = async (campaignId, btnType) => {
    ...
    const failureCallBack = (error) => {
      console.log(JSON.stringify(error), "campaignsubmit error");
      if (error?.response?.data?.data?.length > 0) {
        alert(error?.response?.data?.data[0].message);
      } else if (error?.data?.length > 0) {
        alert(error?.data[0]?.message);
                      ...
                      {selectedBgImg && (
                        <>
                          ...
                          <Slider
                            ...
                            maximumValue={1}
                            onValueChange={(value) => {
                              console.log("setTransparency", value);
                  ...
                  <View style={{ padding: 5 }}>
                    ...
                    <View style={{ marginTop: 8 }}>
                      ...
                      <Slider
                        ...
                        onValueChange={(value) => {
                          console.log(
                            "themeColor.placeHolderregionTransparencyInPercentage",
                            value
                          );
                          ...
                          setSelectedTemplet({ ...selectedTemplet });
                          console.log(
                            'selectedTemplet.regions[selectRegionForEdit]["regionTransparencyInPercentage"]',
                            selectedTemplet.regions[selectRegionForEdit][
                              "regionTransparencyInPercentage"
                            ]
                          );
